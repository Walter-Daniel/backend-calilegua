...
export class OrderDetailService {
  ...
  async create(data: CreateOrderDetailDTO) {
    ...
    if(!order) throw new NotFoundException(`Order with ID ${data.orderId} not found`);
  ...
  async update(id: string, changes: UpdateOrderDetailDTO) {
    ...
    if (changes.orderId && changes.orderId !== orderDetail.order.id) {
      ...
      if (!updatedOrder) {
        throw new NotFoundException(`Order with ID ${changes.orderId} not found`);
      }
    }
    ...
    if (changes.productId && changes.productId !== orderDetail.product.id) {
      updatedProduct = await this.productRepo.findOneBy({ id: changes.productId });
      if (!updatedProduct) {
        throw new NotFoundException(`Product with ID ${changes.productId} not found`);
      }
    }
    ...
    const productPrice = updatedProduct ? updatedProduct.price : orderDetail.product.price;
  ...
  async findOne(id: string) {
    ...
    if(!orderDetail) throw new NotFoundException(`Order detail with ID #${id} not found`);
    return orderDetail;
  }
}
